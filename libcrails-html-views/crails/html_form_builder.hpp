...
namespace Crails
{
  ...
  public:
    ...
    template<typename VALUE, typename METHOD> std::string i18n_text_field(const std::string& name, const VALUE& value, METHOD method, std::map<std::string,std::string> attrs = {}) const { return html_template.i18n_text_field<VALUE>(scoped_name(name), (model.*method)(), attrs); }
    template<typename VALUE, typename METHOD> std::string i18n_text_area(const std::string& name, const VALUE& value, METHOD method, std::map<std::string,std::string> attrs = {}) const { return html_template.i18n_text_area<VALUE>(scoped_name(name), (model.*method)(), attrs); }
